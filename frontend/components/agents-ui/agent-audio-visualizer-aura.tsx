...
export interface AgentAudioVisualizerAuraProps {
  ...
  /**
   * Optional normalized audio level (0-1) for non-LiveKit audio sources.
   * When provided, this is used instead of analyzing the LiveKit track.
   */
  audioLevel?: number;
}
...
export function AgentAudioVisualizerAura({
  ...
  const { speed, scale, amplitude, frequency, brightness } =
    useAgentAudioVisualizerAura(state, audioTrack, audioLevel);
